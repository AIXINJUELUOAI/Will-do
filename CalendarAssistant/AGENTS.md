...
| 你要加的东西 | 先登记到 | 守卫规则 |
|---|---|---|
| 新功能模块（新业务能力） | `shared/management/catalog/FeatureCatalog.kt`（`features` 里加 `FeatureEntry`） | —（registry-check 纳入；AGENTS.md 约定） |
...
| 新流程主线（Pipeline/Orchestrator） | `shared/management/catalog/PipelineCatalog.kt`（`pipelines` 里加 `PipelineEntry`） | —（registry-check 纳入；AGENTS.md 约定） |
| 新辅助工具（Helper/Mapper/Support） | `shared/management/catalog/HelperCatalog.kt`（`helpers` 里加 `HelperEntry`） | —（registry-check 纳入；AGENTS.md 约定） |
| 新策略（Policy） | `shared/management/catalog/PolicyCatalog.kt`（`policies` 里加 `PolicyEntry`） | —（registry-check 纳入；AGENTS.md 约定） |
| 新后台任务（Worker） | `shared/management/catalog/WorkerCatalog.kt`（`workers` 里加 `WorkerEntry`） | —（registry-check 纳入；AGENTS.md 约定） |
