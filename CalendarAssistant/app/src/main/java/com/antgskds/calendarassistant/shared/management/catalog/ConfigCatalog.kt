...
object ConfigCatalog {
    ...
    val items: List<ConfigItem> = listOf(
        ...
        ConfigItem(
            domain = ConfigDomain.RECOGNITION,
            kind = ConfigKind.POLICY,
            key = "recognition.sms_pickup.dedup_enabled",
            label = "短信取件码去重",
            description = "开启后相同短信取件码只入库一次；关闭后不检查已有短信取件日程。",
            exposure = ConfigExposure.DEVELOPER_ONLY,
            control = ConfigControl.Toggle,
            get = { if (it.smsPickupDedupEnabled) 1 else 0 },
            set = { s, v -> s.copy(smsPickupDedupEnabled = v != 0) },
        ),
        ConfigItem(
