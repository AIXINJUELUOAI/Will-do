...
object WeatherRiskAnalyzer {
    fun analyze(hourly: List<WeatherHourlyForecast>, lookaheadHours: Int, settings: MySettings): List<WeatherRiskAlert> {
        ...
        selectedHours
            .forEachIndexed { index, hour ->
                val candidate = riskForHour(hour, index, selectedHours, settings) ?: return@forEachIndexed
    ...
    private fun riskForHour(hour: WeatherHourlyForecast, index: Int, hourly: List<WeatherHourlyForecast>, settings: MySettings): RiskCandidate? {
        val text = hour.text
        ...
        val temperatureDrop = temperatureDropBefore(hourly, index, temp, settings)

        val type = when {
            text.contains("高温") || (temp != null && temp >= settings.weatherHighTempTriggerCelsius) -> RiskType(
                category = "heat",
                title = "高温",
                level = if (temp != null && temp >= settings.weatherHighTempHighCelsius) "high" else "medium",
                advice = "请减少长时间户外活动，注意补水和防暑降温。"
            )
            text.contains("寒潮") || text.contains("强降温") || temperatureDrop >= settings.weatherStrongCoolingDropCelsius -> RiskType(
                category = "cold",
                title = if (text.contains("寒潮")) "寒潮" else "强降温",
                level = if (text.contains("寒潮") || temperatureDrop >= settings.weatherSevereCoolingDropCelsius) "high" else "medium",
                advice = "请及时添衣保暖，留意温差变化和出行影响。"
            )
            text.contains("低温") || text.contains("寒冷") || text.contains("霜冻") || text.contains("冰冻") || (temp != null && temp <= settings.weatherLowTempTriggerCelsius) -> RiskType(
                category = "cold",
                title = when {
                    text.contains("霜冻") -> "霜冻"
                    text.contains("冰冻") -> "冰冻"
                ...
                level = if (temp != null && temp <= settings.weatherLowTempHighCelsius) "high" else "medium",
            ...
            text.contains("雨") || precip >= settings.weatherRainTriggerPrecipTenthMm / 10.0 || pop >= settings.weatherRainTriggerPopPercent -> RiskType(
                category = "rain",
                title = rainTitle(text, precip, settings),
                level = when {
                    text.contains("暴雨") || precip >= settings.weatherRainHighPrecipTenthMm / 10.0 -> "high"
                    text.contains("中雨") || text.contains("大雨") || precip >= settings.weatherRainMediumPrecipTenthMm / 10.0 || pop >= settings.weatherRainMediumPopPercent -> "medium"
                    else -> "low"
                },
                advice = "建议出门带伞，注意路面湿滑和通勤延误。"
            )
            text.contains("台风") || text.contains("大风") || windScaleMax >= settings.weatherWindScaleTrigger -> RiskType(
                category = "wind",
                title = if (text.contains("台风")) "台风" else "大风",
                level = if (text.contains("台风") || windScaleMax >= settings.weatherWindScaleMedium) "medium" else "low",
    ...
    private fun rainTitle(text: String, precip: Double, settings: MySettings): String {
        return when {
            text.contains("特大暴雨") -> "特大暴雨"
            text.contains("大暴雨") -> "大暴雨"
            text.contains("暴雨") || precip >= settings.weatherRainHighPrecipTenthMm / 10.0 -> "暴雨"
            text.contains("大雨") -> "大雨"
            text.contains("中雨") -> "中雨"
            text.contains("阵雨") -> "阵雨"
            text.contains("小雨") -> "小雨"
    ...
    private fun temperatureDropBefore(hourly: List<WeatherHourlyForecast>, index: Int, temp: Int?, settings: MySettings): Int {
        if (temp == null || index <= 0) return 0
        val start = (index - settings.weatherCoolingLookbackHours).coerceAtLeast(0)
