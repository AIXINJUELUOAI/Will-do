...
object AiPrompts {
    ...
    fun getSchedulePrompt(timeStr: String, dateToday: String, dateYesterday: String, dateBeforeYesterday: String): String {
        ...
        return """
            ...
            【交通场景特殊规则 - 必须严格遵守】
            当识别到火车票、高铁票或网约车/出租车/导航信息时，必须遵守以下微格式协议：
            ...
            2. **网约车/出租车/导航**：
               - **【关键逻辑：起终点判定】**：
                 OCR文本中通常会有 "起/始" 和 "终/止" 字样。
                 - 必须将 "终"、"目的地"、"下车" 附近的地点识别为【目的地】。
                 - 必须将 "起"、"出发"、"上车" 附近的地点识别为【出发地】。
                 - 若出现 "接机" 字样，通常 "终" 是目的地（如家里或酒店），"起" 是机场。
               
               - **字段填充规则**：
                 - **title**：必须填 "前往 [目的地]"。 (例如："前往 沈阳北站")。
                 - **location**：必须填 [目的地]。
                 - **description**：
                    格式：【用车】车型颜色|车牌号
                    示例：【用车】黑色奔驰|辽A88888
            ...
            纯 JSON 对象：
            {
              "reasoning": "必须写出：基准是哪天？是否识别到'起/终'关键字？目的地判定为哪里？",
              "events": [ $itemSchema ]
            }
        """.trimIndent()
    }
