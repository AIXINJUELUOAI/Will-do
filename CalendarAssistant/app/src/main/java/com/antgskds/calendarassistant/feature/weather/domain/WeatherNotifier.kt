...
class WeatherNotifier(context: Context) {
    ...
    fun notifyAlerts(
        ...
        if (!canNotify()) return
        val weatherTimeoutMs = app?.settingsQueryApi?.settings?.value?.weatherNotificationTimeoutMs?.toLong()

        alerts.forEach { alert ->
            val title = WeatherNormalDisplay.officialTitle(WeatherWarningText.officialTitle(alert))
            val content = WeatherNormalDisplay.officialContent(alert.description, alert.instruction, alert.eventName)
            notifyOfficialOnce(locationName, alert, title) {
                if (showLiveNotification) {
                    ...
                    app?.notificationCenter?.publishPlainNotification(
                        weatherRequest(
                            key = NotificationKey.weatherAlert("official:${stableId(title)}"),
        ...
        risks.forEach { risk ->
            val title = WeatherNormalDisplay.riskTitle(risk.title)
            val content = WeatherNormalDisplay.riskContent(risk.message, risk.weatherText)
            notifyRiskOnce(locationName, risk, title) {
                if (showLiveNotification) {
                    ...
                    app?.notificationCenter?.publishPlainNotification(
                        weatherRequest(
                            key = NotificationKey.weatherAlert("risk:${riskEventBucket(risk)}"),
                            notificationId = notificationId,
                            title = title,
                            content = content,
