...
object DebugActionRegistry {
    ...
    val actions: List<DebugAction> = listOf(
        ...
        DebugAction("test-capsule-quickmemo-pinned", "测试胶囊·随口记挂起", CATEGORY_CAPSULE) { app -> fireTextQuickMemoPinned(app) },
    ...
    private fun fireTextQuickMemoPinned(app: App) {
        if (!liveCapsuleReady(app)) return
        val memoText = "今天下班前把初始化页实况胶囊动画调顺。"
        app.capsuleCenter.showTextQuickMemo(
            memoId = DEBUG_TEXT_QUICK_MEMO_ID,
            title = memoText,
            durationMs = 60_000L
        )
        Log.d(DEBUG_TAG, "capsule text quick memo pinned fired")
    }
    ...
    private fun clearTestCapsules(app: App) {
        ...
        app.capsuleCenter.clearTextQuickMemo()
    ...
    private const val DEBUG_TEXT_QUICK_MEMO_ID = 990103L
}
