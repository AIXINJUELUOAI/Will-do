 ...
 * 都应只依赖本接口，而不是直接拿 [com.antgskds.calendarassistant.feature.schedule.application.sync.CalendarSyncService] 实现类。
 * 同步是外部副作用：允许失败/重试，失败不回滚本地入库。
 *
 * 由 CalendarSyncService 实现。签名与其现有实现一致（纯增量契约，不改行为）。
 */
interface SyncApi {
